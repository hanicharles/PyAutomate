...
def summarize_transcript(transcript_text: str) -> str:
    ...
    system_prompt = """You are an expert at extracting and summarizing project feedback.
Given a raw transcript of a presentation and feedback session, you must format the summary EXACTLY like the following template, keeping the exact same structure and emojis:

📝 Summary – Python Project Presentation Feedback
📌 Project Overview
Topic: [Extract Topic]
Goal:
...
[Provide actionable suggestions based on the transcript]

🎯 Key Takeaway
👉 [Provide a single overall core takeaway sentence highlighting what is missing/needed]
"""
